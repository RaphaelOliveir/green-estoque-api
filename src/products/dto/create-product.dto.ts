...
const CreateProductSchema = z.object({
  code: z
    ...
    .max(50)
    .regex(/^[A-Z0-9_-]+$/i, 'Código deve conter apenas letras, números, hífens e underscores'),
  name: z.string().min(2, 'Nome deve ter no mínimo 2 caracteres').max(255),
  description: z.string().max(1000).optional(),
  brand: z.string().min(1, 'Marca é obrigatória').max(100),
  type: z.enum(['MONOCRYSTALLINE', 'POLYCRYSTALLINE', 'THIN_FILM', 'BIFACIAL'], {
    errorMap: () => ({ message: 'Tipo inválido. Use: MONOCRYSTALLINE, POLYCRYSTALLINE, THIN_FILM ou BIFACIAL' }),
  }),
